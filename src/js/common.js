...
forEach(document.querySelectorAll('[data-slider]'), function(slider) {
  ...
  const move = (index, dir) => {
    ...
    let callback = (retreat, active, progress) => {
      ...
      if (params.vertical) {
        let height = dir < 0 ? elItems[retreat].offsetHeight : elItems[active].offsetHeight
        let y = dir < 0 ? height * progress : height - (height * progress)
        elWrapper.style.transform = `translate3d(0px, -${y}px, 0px)`
      } else {
        let width = dir < 0 ? elItems[retreat].offsetWidth : elItems[active].offsetWidth
    ...
    timeline.add(callback.bind(this, retreat, active), {
      onStart: slider.dispatchEvent(new CustomEvent('slide.start', { detail: { retreat, active } })),
      onEnd: slider.dispatchEvent(new CustomEvent('slide.end', { detail: { retreat, active } }))
    })
    timeline.play()
  }
